...
//         ? process.env. PUBLIC_STRIPE_ESSENTIAL_PRICE_ID
...
export default function AccountPage() {
    ...
    try {
      ...
      if (response.ok) {
        if (result.message && result.message.includes('Development mode')) {
          ...
          setRedirecting(true) // show spinner while redirecting
          window.location.href = result.checkoutUrl
        } else {
          setError('No checkout URL received')
        }
  ...
  // Show full-page spinner if loading or redirecting
  if (loading || redirecting) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-b-4 border-blue-600"></div>
        {redirecting && (
          <p className="mt-4 text-gray-600 text-center">Redirecting to checkout...</p>
        )}
      </div>
    )
  }
